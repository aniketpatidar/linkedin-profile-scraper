...
export function normalizeProfileUrl(value) {
  ...
  try {
    ...
    throw new ProfileApiError(
      "invalid_url",
      "url must be a valid HTTPS LinkedIn URL",
      400,
    );
  }
  ...
  if (
    ...
    !LINKEDIN_HOSTS.has(parsed.hostname.toLowerCase())
  ) {
    throw new ProfileApiError(
      "unsupported_url",
      "url must be an HTTPS LinkedIn profile URL",
      422,
    );
  }
  ...
  if (!match) {
    throw new ProfileApiError(
      "unsupported_profile",
      "only public member profiles are supported",
      422,
    );
  }
...
export function createProfileWorker(
  ...
  { rateLimiter = createRateLimiter(), logger = () => {} } = {},
) {
  return {
    async fetch(request) {
      ...
      if (url.pathname !== "/profile" || request.method !== "POST") {
        return finish(
          errorResponse(
            new ProfileApiError("not_found", "route not found", 404),
            requestId,
          ),
        );
      }
      ...
      try {
        ...
        return finish(
          Response.json(normalizeProfile(profile, profileUrl), {
            headers: { "x-request-id": requestId },
          }),
        );
      } catch (error) {
        return finish(errorResponse(error, requestId));
      }
    },
  };
}
